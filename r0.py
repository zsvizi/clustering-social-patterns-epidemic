...
def main():
    ...
    for root, dirs, files in os.walk("../sens_data"):
        for filename in files:
            filename_without_ext = os.path.splitext(filename)[0]
            _, country, susc, r0, contact = tuple(filename_without_ext.split("_"))

            r0generator = R0Generator(param=data_loader.model_parameters_data)

            susceptibility = np.array([1.0] * 16)
            susceptibility[:4] = float(susc)
            r0generator.parameters.update({"susc": susceptibility})

            contact_matrix = data_loader.contact_data[country]["home"] + data_loader.contact_data[country]["work"] + \
                             data_loader.contact_data[country]["school"] + data_loader.contact_data[country]["other"]
            contact_home = data_loader.contact_data[country]["home"]

            # if contact == "home":
            #     contact_mtx2 = contact_home
            # elif contact == "normed":
            #     contact_mtx2 = contact_matrix - np.min(contact_matrix - contact_home)
            # else:
            #     raise Exception("No contact type")

            eig1 = r0generator.get_eig_val(contact_mtx=contact_matrix)

            print("\"" + filename + "\") ./covid_" + country + " $1 " + str(float(r0) / eig1) + " $SUSC >results/$FN")
            print(";;")
            # print("Beta =", float(r0) / eig1)
            # eig2 = r0generator.get_eig_val(contact_mtx=contact_mtx2)

            # print("Dominant eigenvalue = " + str(eig))
            # print("R0 =", str(float(r0) * (eig2 / eig1)))
